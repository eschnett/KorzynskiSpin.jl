...
\author{Notes for the \texttt{KorzyńskiSpin.jl} package}
\date{\today}
...
two-dimensional Ricci flow, obtain the M\"obius generators directly in the
...
\item A ring denotes quantities of the \emph{unit-round representative}
  $\qo_{AB}$ produced by the Ricci flow (area $4\pi$, $R[\qo]=2$):
...
The flow in Section~\ref{sec:ricciflow} needs $R[q]$.  In any chart,
\begin{equation}
...
\subsection{Uniformization by normalized Ricci flow}
\label{sec:ricciflow}
...
The linearization of \eqref{eq:flowscalar} is
$\partial_t\,\delta u = \ee^{-2u}\bar\Delta\,\delta u - R(t)\,\delta u$,
so explicit integrators need
$\Delta t \lesssim c\,\min(\ee^{2u})\,/\,\ell_{\max}(\ell_{\max}+1)$
...
$\kappa \approx \max(\ee^{-2u}\,\lambda_{\bar q})$ chosen for stability.
Monitor $\lVert R(t)-2\rVert_\infty$ and stop when it reaches the requested
tolerance; also monitor $\oint \ee^{2u}\bar\epsilon = 4\pi$ (preserved by
\eqref{eq:flowscalar} up to time-discretization error; renormalize $u$ by a
constant if it drifts).

\paragraph{Newton polish (recommended).}
...
($(\Lapo+2)\,c = 2c$) and $\ell\ge2$ modes are negative.  In practice:
...
\eqref{eq:flowscalar} until $\lVert R-2\rVert_\infty \lesssim 10^{-2}$ (the
...
The operators $\Delta_q$ (for \eqref{eq:hodgesolve}), the Newton operator
$\bar\Delta + 2\ee^{2u}$ (for \eqref{eq:liouville}), and $\bar\Delta + 2
\ee^{2u_\infty}$ (shift-invert in \eqref{eq:eigenproblem}) are all of the
...
\item Ricci flow / Newton: $\lVert R[\qo]-2\rVert_\infty$, area drift;
...
\item \textbf{Normalize.}  $\bar q = (4\pi/\mathcal{A})\,q$; compute
  $\bar R$ \ \eqref{eq:Reth}.
\item \textbf{Uniformize.}  Ricci flow \eqref{eq:flowscalar} (optionally
  Newton polish \eqref{eq:liouville}) $\to u_\infty$,
