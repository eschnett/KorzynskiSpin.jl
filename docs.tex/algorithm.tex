...
\subsection{Balanced round metric and horizon multipoles}
\label{sec:multipoles}

The spin, the axis and the axial field do not depend on which member of the
M\"obius family of unit round metrics the fast flow of
Section~\ref{sec:ricciflow} happens to reach.  (The parallel frame fixes two
of the three boost parameters; a boost along the axis leaves $\vJ'$, $\vK'$
and $\hat a_i\tilde\phi_i$ unchanged.)  Multipole moments, however, are
integrals against the spherical harmonics $\mathring Y_{\ell m}$ of $\qo$, and
these \emph{do} change under the residual M\"obius freedom.  Following
Ashtekar, Khera, Kolanowski and Lewandowski \cite{akkl2022} we fix it by
requiring that the \emph{area dipole} vanish:
\begin{equation*}
  d_i \equiv \oint \chi_i\,\epsilon = 0 ,
\end{equation*}
with the physical area form $\epsilon$.  For any metric on $S^2$ exactly one
unit round metric conformal to $q$ satisfies this (\cite{akkl2022}, App.~A);
if $q$ has an axial Killing field, that field is also a Killing field of the
balanced $\qo$.  The condition is the analogue of a centre-of-mass frame,
with the area density in the role of the mass.

\paragraph{M\"obius action without a new eigenproblem.}
...
eigenfunctions, now of $\Lapo'$ (for a boost with velocity $\vec b$ this is
...
\paragraph{Balancing by Newton iteration.}
...
\cite{akkl2022}, eq.~(A.7).  The iteration therefore converges globally, and
...
dipole $\vec L_1 = \tfrac12\oint \dd\chi\wedge\omega$.  It points $\hat x$
...
$(\vartheta,\varphi) = (\arccos\chi_3, \atantwo(\chi_2,\chi_1))$, the
shape and current multipoles are (\cite{akkl2022,glc2026}; on a
non-expanding horizon $\operatorname{Re}\Psi_2 = -R/4$ and
$\operatorname{Im}\Psi_2\,\epsilon = \tfrac12\dd\omega$)
\begin{equation*}
  ...
  L_{\ell m} = -\oint \operatorname{Im}\Psi_2\,\mathring Y_{\ell m}\,\epsilon
             = \frac12\oint \dd\mathring Y_{\ell m}\wedge\omega^{\mathrm{inv}} .
\end{equation*}
...
handedness integral \eqref{eq:handedness}.  The monopoles are universal,
...
current moments of $R[q]$ and $\dd\omega$.  Two further consistency checks:
...
form \cite{glc2026}, namely
$\chi_3 = z(\zeta) = \tanh(\artanh\zeta - \beta^2\zeta)$, with
$\zeta = \cos\theta$ and $\beta^2 = a^2/(r_+^2+a^2)$, and
\begin{equation*}
  I_\ell + iL_\ell = \tfrac12 (1+\hat a^2)^2\sqrt{(2\ell+1)\pi}
  \int_{-1}^{1} \frac{P_\ell(z(\zeta))}{(1-i\hat a\zeta)^3}\,\dd\zeta ,
  \qquad \hat a = a/r_+ ,
\end{equation*}
...
\item \textbf{Multipoles (optional).}  Balance the round metric,
  $\oint\chi_i\,\epsilon = 0$, by Newton iteration on the pointwise M\"obius
  action; orient $\hat z\parallel\vec L_1$; evaluate $I_{\ell m}$,
  $L_{\ell m}$ (Section~\ref{sec:multipoles}).
\end{enumerate}
...
\item \textbf{Distorted non-axisymmetric data.}  E.g.\ Bowen--York or
  ...
  changes.
\item \textbf{Multipoles.}  Kerr: the balanced $\chi_3$ and $I_\ell$,
  $L_\ell$ against the closed forms of \cite{glc2026}, including on boosted
  slicings, whose horizon cross-sections are different but isometric.  A
  deformed surface in a M\"obius-distorted chart: the multipoles do not depend
...
\bibitem{glc2026}
E.~Gourgoulhon, A.~Le~Tiec, M.~Casals,
\emph{Horizon multipole moments of a Kerr black hole} (2026);
\href{https://arxiv.org/abs/2602.05823}{arXiv:2602.05823 [gr-qc]}.
